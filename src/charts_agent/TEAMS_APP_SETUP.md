# Connect charts-agent to Microsoft Teams

`azd deploy` already did the Azure side for you:

- Azure Bot: `charts-agent-bot-ba85c04e` (Microsoft Teams channel enabled)
- Bot ID (msaAppId): `d7f0fada-a2db-4688-9503-00f434381bd9`  <- you will paste this as the bot id
...
3. Left menu **App features** -> **Bot** -> **Select an existing bot** -> enter the
   Bot ID `d7f0fada-a2db-4688-9503-00f434381bd9`, tick the **Personal** scope, then **Save**.
...
```json
{
  ...
  "name": { "short": "charts-agent", "full": "charts-agent" },
  "description": { "short": "charts-agent agent", "full": "charts-agent agent on Microsoft Teams" },
  "icons": { "color": "color.png", "outline": "outline.png" },
  "accentColor": "#FFFFFF",
  "bots": [{ "botId": "d7f0fada-a2db-4688-9503-00f434381bd9", "scopes": ["personal"] }]
}
```
...
```sh
zip -j charts-agent-teams-app.zip manifest.json color.png outline.png          # bash
```
```powershell
Compress-Archive manifest.json,color.png,outline.png charts-agent-teams-app.zip # PowerShell
```
...
atk install --file-path charts-agent-teams-app.zip --scope Personal
```
